...
interface MicRecorderProps {
  ...
  size?: 'default' | 'large' // 'large' for mobile Google Translate style (h-28 w-28)
}
...
const MicRecorderWithWaveform: React.FC<MicRecorderProps> = ({
  ...
  /* -------------------------- view ---------------------------- */
  const sizeClasses = size === 'large' ? 'h-28 w-28' : 'h-20 w-20'
  const iconClasses = size === 'large' ? 'h-12 w-12' : 'h-9 w-9'
  ...
  return (
    <div className="flex flex-col items-center">
      <button
        ...
        style={style}
        className={`
          flex items-center justify-center ${sizeClasses} rounded-full
        ...
        `}
      >
        <Mic className={`${iconClasses} pointer-events-none`} />
      </button>
