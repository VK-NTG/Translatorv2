#!/usr/bin/env python3
"""
Populate language contexts via API calls to the running Flask application.
...
def populate_contexts():
    """Populate language contexts via API calls."""
    ...
    print("🌍 Populating language contexts via API...")
    ...
    for context_data in LANGUAGE_CONTEXTS:
        ...
        try:
            response = requests.get(
                f"{API_BASE}/context/language-contexts/{language_code}",
                headers=headers
            )
            ...
            if response.status_code == 200:
                ...
                if update_response.status_code == 200:
                    updated_count += 1
                    print(f"✅ Updated {language_code} - {context_data['language_name']}")
                else:
                    print(f"❌ Failed to update {language_code}: {update_response.status_code}")

            elif response.status_code == 404:
                # Create new context
                create_response = requests.post(
                    f"{API_BASE}/context/language-contexts",
                    headers=headers,
                    json=context_data
                )
                ...
                if create_response.status_code == 201:
                    created_count += 1
                    print(f"🆕 Created {language_code} - {context_data['language_name']}")
                else:
                    print(f"❌ Failed to create {language_code}: {create_response.status_code}")
                    print(f"Response: {create_response.text}")
            else:
                print(f"❌ Error checking {language_code}: {response.status_code}")

        except requests.exceptions.RequestException as e:
            print(f"❌ Network error for {language_code}: {e}")

    print(f"\n🎉 Population complete!")
    print(f"📊 Created: {created_count} contexts")
    print(f"📊 Updated: {updated_count} contexts")
    print(f"📊 Total processed: {len(LANGUAGE_CONTEXTS)} contexts")

    return created_count + updated_count > 0

if __name__ == "__main__":
    print("🚀 Language Context Population Script (API Version)")
    print("=" * 55)
